...
class MarkersController {
  ...
  private double defaultAnimationSpeed = 60.0;
  private int defaultAnimationMaxDuration = 5000;
  private MarkerController.OnMarkerMovedListener markerMovedListener;
  
  public interface OnMarkerRemovedListener {
    void onMarkerRemoved(String markerId);
  }
  private OnMarkerRemovedListener markerRemovedListener;
  ...
  void setAnimationDefaults(double speed, int maxDuration) {
    this.defaultAnimationSpeed = speed;
    this.defaultAnimationMaxDuration = maxDuration;
    for (MarkerController controller : markerIdToController.values()) {
      controller.setAnimationDefaults(speed, maxDuration);
    }
  }

  void setOnMarkerMovedListener(MarkerController.OnMarkerMovedListener listener) {
    this.markerMovedListener = listener;
    for (MarkerController controller : markerIdToController.values()) {
      controller.setOnMarkerMovedListener(listener);
    }
  }

  void setOnMarkerRemovedListener(OnMarkerRemovedListener listener) {
    this.markerRemovedListener = listener;
  }
  ...
  void addMarkers(@NonNull List<PlatformMarker> markersToAdd) {
    ...
    Toast.makeText(context, "Added " + markersToAdd.size() + " markers", Toast.LENGTH_SHORT).show();
  }
  ...
  void changeMarkers(@NonNull List<PlatformMarker> markersToChange) {
    ...
    Toast.makeText(context, "Changed " + markersToChange.size() + " markers", Toast.LENGTH_SHORT).show();
  }
  ...
  void removeMarkers(@NonNull List<String> markerIdsToRemove) {
    ...
    for (String markerId : markerIdsToRemove) {
      ...
      if (markerRemovedListener != null) {
        markerRemovedListener.onMarkerRemoved(markerId);
      }
    }

    Toast.makeText(context, "Removed " + markerIdsToRemove.size() + " markers", Toast.LENGTH_SHORT).show();
  }
  ...
  private void createControllerForMarker(String markerId, Marker marker, boolean consumeTapEvents) {
    ...
    controller.setAnimationDefaults(defaultAnimationSpeed, defaultAnimationMaxDuration);
    if (markerMovedListener != null) {
      controller.setOnMarkerMovedListener(markerMovedListener);
    }
