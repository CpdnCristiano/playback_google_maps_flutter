...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PlatformMarker {
      ...
      val speed = pigeonVar_list[14] as Double?
      val maxDuration = pigeonVar_list[15] as Long?
      return PlatformMarker(alpha, anchor, consumeTapEvents, draggable, flat, icon, infoWindow, position, rotation, visible, zIndex, markerId, clusterManagerId, collisionBehavior, speed, maxDuration)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun hashCode(): Int {
    ...
    result = 31 * result + MessagesPigeonUtils.deepHash(this.maxDuration)
    return result
  }
}
...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): PlatformMapConfiguration {
      ...
      val animationSpeed = pigeonVar_list[21] as Double
      val animationMaxDuration = pigeonVar_list[22] as Long
      return PlatformMapConfiguration(compassEnabled, cameraTargetBounds, mapType, minMaxZoomPreference, mapToolbarEnabled, rotateGesturesEnabled, scrollGesturesEnabled, tiltGesturesEnabled, trackCameraPosition, zoomControlsEnabled, zoomGesturesEnabled, myLocationEnabled, myLocationButtonEnabled, padding, indoorViewEnabled, trafficEnabled, buildingsEnabled, liteModeEnabled, markerType, mapId, style, animationSpeed, animationMaxDuration)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun hashCode(): Int {
    ...
    result = 31 * result + MessagesPigeonUtils.deepHash(this.animationSpeed)
    result = 31 * result + MessagesPigeonUtils.deepHash(this.animationMaxDuration)
    return result
  }
}
...
interface MapsApi {
   ...
   */
  fun followMarker(markerId: String?)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: MapsApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.google_maps_flutter_android.MapsApi.followMarker$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val markerIdArg = args[0] as String?
            val wrapped: List<Any?> = try {
              api.followMarker(markerIdArg)
              listOf(null)
            } catch (exception: Throwable) {
              MessagesPigeonUtils.wrapError(exception)
            }
  ...
  fun getTileOverlayTile(tileOverlayIdArg: String, locationArg: PlatformPoint, zoomArg: Long, callback: (Result<PlatformTile>) -> Unit)
{
  ...
  fun onMarkerUpdateToast(messageArg: String, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.google_maps_flutter_android.MapsCallbackApi.onMarkerUpdateToast$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
