...
- (void)updateFromPlatformMarker:(FGMPlatformMarker *)platformMarker
                   assetProvider:(NSObject<FGMAssetProvider> *)assetProvider
                    ...
                    defaultSpeed:(double)defaultSpeed
              defaultMaxDuration:(NSInteger)defaultMaxDuration {
  ...
  NSTimeInterval duration = [FGMMarkerController updateMarker:self.marker
  ...
  if (duration > 0 && self.onMarkerMoved) {
    self.onMarkerMoved(self.markerIdentifier, self.marker.position, duration);
  }
}
...
+ (void)updateMarker:(GMSMarker *)marker
  ...
  CLLocationCoordinate2D newPosition = FGMGetCoordinateForPigeonLatLng(platformMarker.position);
  CLLocationCoordinate2D oldPosition = marker.position;
  float newRotation = (float)platformMarker.rotation;
  float oldRotation = (float)marker.rotation;

  BOOL positionChanged = (newPosition.latitude != oldPosition.latitude ||
                          newPosition.longitude != oldPosition.longitude);
  BOOL rotationChanged = (newRotation != oldRotation);

  NSTimeInterval duration = 0;
  if (positionChanged || rotationChanged) {
    CLLocationDistance distance = GMSGeometryDistance(oldPosition, newPosition);
    
    double speed = platformMarker.speed ? platformMarker.speed.doubleValue : defaultSpeed;
    NSInteger maxDurationMs = platformMarker.maxDuration ? platformMarker.maxDuration.integerValue : defaultMaxDuration;
    
    NSTimeInterval duration = distance / speed;
    NSTimeInterval maxDuration = (NSTimeInterval)maxDurationMs / 1000.0;
...
@property(nonatomic) double defaultSpeed;
@property(nonatomic) NSInteger defaultMaxDuration;
...
- (instancetype)initWithMapView:(GMSMapView *)mapView
  ...
  if (self) {
    ...
    _defaultSpeed = 60.0;
    _defaultMaxDuration = 5000;
  }
  return self;
}


- (void)setAnimationDefaultsWithSpeed:(double)speed maxDuration:(NSInteger)maxDuration {
  self.defaultSpeed = speed;
  self.defaultMaxDuration = maxDuration;
}
...
- (void)addMarker:(FGMPlatformMarker *)markerToAdd {
  ...
  [controller updateFromPlatformMarker:markerToAdd
                    ...
                    defaultMaxDuration:self.defaultMaxDuration];
...
- (void)removeMarker:(NSString *)identifier {
  ...
  if (self.onMarkerRemoved) {
    self.onMarkerRemoved(identifier);
  }
}
