...
@property(nonatomic, copy) NSString *followedMarkerId;
...
- (instancetype)initWithMapView:(GMSMapView *_Nonnull)mapView
  ...
  if (self = [super init]) {
    ...
    _markersController.onMarkerMoved =
        ^(NSString *markerId, CLLocationCoordinate2D newPosition, NSTimeInterval duration) {
          if ([markerId isEqualToString:weakSelf.followedMarkerId]) {
            [weakSelf.mapView animateToLocation:newPosition];
          }
        };
    _markersController.onMarkerRemoved = ^(NSString *markerId) {
      if ([markerId isEqualToString:weakSelf.followedMarkerId]) {
        weakSelf.followedMarkerId = nil;
      }
    };
...
- (void)interpretMapConfiguration:(FGMPlatformMapConfiguration *)config {
  ...
  [self.markersController setAnimationDefaultsWithSpeed:config.animationSpeed
                                            maxDuration:config.animationMaxDuration];
}
...
- (void)followMarkerWithIdentifier:(nullable NSString *)markerId
                            error:(FlutterError *_Nullable __autoreleasing *_Nonnull)error {
  self.controller.followedMarkerId = markerId;
}
